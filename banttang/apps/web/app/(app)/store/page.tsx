...
const SECTION_META: Record<FeedSection, { emoji: string; title: string; shortLabel: string }> = {
  delivery: { emoji: "🍽️", title: "우리 동네 배달 맛집", shortLabel: "배달" },
  grocery: { emoji: "🛒", title: "1인 가구 장보기", shortLabel: "장보기" },
  household: { emoji: "🧴", title: "자취 생활템", shortLabel: "생활템" },
  local_news: { emoji: "📰", title: "동네 소식", shortLabel: "동네소식" },
};
const SECTION_ORDER: FeedSection[] = ["delivery", "grocery", "household", "local_news"];
...
export default async function StorePage() {
  ...
  const sections: StoreSection[] = SECTION_ORDER.map((key) => ({
    ...
    shortLabel: SECTION_META[key].shortLabel,
    emoji: SECTION_META[key].emoji,
    cards: feed.sections[key] ?? [],
  })).filter((s) => s.cards.length > 0);

  if (sections.length === 0) {
    return <EmptyState message={"아직 추천할 정보가 없어요.\n곧 채워질 거예요!"} />;
  }
  ...
  return (
    <main className="flex flex-1 flex-col px-4 py-5">
      ...
      <StoreFeedTabs sections={sections} />
    </main>
  );
}
